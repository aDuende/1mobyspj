...
export default function SeriesLayout({ item }: SeriesLayoutProps) {
  ...
  const progress =
    totalEps > 0 ? Math.round((totalCompleted / totalEps) * 100) : 0;
      ...
      {playing && (
        <div className="flex-1 flex flex-col overflow-auto">
          <div className="p-4 lg:p-6">
            ...
            <h1
              className="text-[17px] font-semibold text-gray-900 dark:text-white mb-4"
              style={GEO}
            >
              {playing.title}
            </h1>
            ...
            {completed.has(playing.id) && (
              <div
                className="flex items-center gap-2 text-emerald-500 text-[12px] font-medium"
                style={GEO}
              >
                <CheckCircle2 className="w-4 h-4" />
                Completed
      ...
      {!playing && (
        <div className="flex-1 overflow-auto">
          <div className="max-w-4xl mx-auto p-4 lg:p-8">
            {/* Thumbnail hero */}
            <div
              className="relative w-full aspect-video rounded-2xl overflow-hidden bg-black shadow-[0_20px_50px_-12px_rgba(0,0,0,0.25)] mb-6 group cursor-pointer"
              onClick={() => {
                const first = allEpisodes[0];
                if (first) setPlaying(first);
              }}
            >
              <img
                src={item.thumbnail}
            ...
            <h3
              className="text-[22px] text-left font-semibold text-gray-900 dark:text-white mb-2"
              style={GEO}
            >
              {item.title}
            </h3>
            <p
              className="text-[13px] text-left text-gray-600 dark:text-gray-300 leading-relaxed mb-4"
              style={GEO}
            >
              {item.description}
            </p>
            ...
            <div className="flex flex-col gap-3">
              {modules.map((mod) => {
                const modDone = mod.episodes.every((ep) =>
                  completed.has(ep.id),
                );
                const isOpen = expanded.has(mod.id);
                const firstUnlocked = mod.episodes.find(
                  (ep) => !completed.has(ep.id) && isUnlocked(ep),
                );
