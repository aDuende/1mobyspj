...
function CompetencyTable({
  ...
  return (
    <div className="mb-6">
      <div
        className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[11px] font-semibold mb-3 ${color}`}
        style={{ fontFamily: '"Geometrica", sans-serif' }}
      >
        {title}
      </div>
      <div className="w-full rounded-2xl overflow-hidden border border-gray-200/60 dark:border-white/5 shadow-[inset_0_1px_4px_rgba(0,0,0,0.06)]">
        ...
        <div className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-0 bg-gray-50/80 dark:bg-gray-900/60 border-b border-gray-200/60 dark:border-white/5">
          {["Competency", "Expected", "Manager", "GAP", "Development"].map(
            (h) => (
              <div
                key={h}
                className="px-4 py-2.5 text-[11px] font-semibold text-gray-500 dark:text-gray-400 text-left"
                style={{ fontFamily: '"Geometrica", sans-serif' }}
              >
                {h}
              </div>
        ...
        {rows.map((row, i) => {
          ...
          return (
            ...
            >
              <div
                className="px-4 py-3 text-[12px] font-medium text-gray-800 dark:text-gray-200 text-left"
                style={{ fontFamily: '"Geometrica", sans-serif' }}
              >
                {row.name}
              </div>
              ...
              <div className="px-4 py-3 flex items-center">
                <span
                  className={`text-[12px] font-bold tabular-nums ${gap < 0 ? "text-rose-500" : gap > 0 ? "text-emerald-500" : "text-blue-500"}`}
                  style={{ fontFamily: '"Geometrica", sans-serif' }}
                >
                  {gap > 0 ? `+${gap}` : gap}
                </span>
              ...
              <div className="px-4 py-3 flex items-center">
                <span
                  className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium ${dev.color} ${dev.bg}`}
                  style={{ fontFamily: '"Geometrica", sans-serif' }}
                >
                  {dev.icon}
                  {dev.label}
      ...
      <div className="p-6 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8" style={fadeUp(80)}>
          {[
            ...
            <div
              key={s.label}
              className={`p-4 rounded-2xl border  ${s.bg} ${s.border} flex items-center gap-3`}
            >
        ...
        {/* Legend */}
        <div
          className="flex flex-wrap items-center gap-4 mb-6 text-[11px] text-gray-500 dark:text-gray-400"
          style={fadeUp(120)}
        >
            ...
            <div key={s} className="flex items-center gap-1.5">
              <div className="flex gap-0.5">
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className={`w-2 h-2 rounded-full ${i <= s ? "bg-[#fc4c02]" : "bg-gray-200 dark:bg-gray-700"}`}
                  />
                ))}
              </div>
