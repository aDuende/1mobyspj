...
const functionalItems: CompetencyRow[] = [
  ...
  {
    category: "Functional",
    name: "FC03 · Strategic Thinking",
    expected: 3,
    manager: 3,
  },
  {
    category: "Functional",
    name: "FC04 · Project Execution",
    expected: 4,
    manager: 4,
  },
  {
    category: "Functional",
    name: "FC05 · Process Improvement",
    expected: 3,
    manager: 2,
  },
];
...
function getDevelopment(gap: number): {
  label: string;
  color: string;
  bg: string;
  ...
  return (
    <div className="flex items-center justify-center gap-1.5">
      {[1, 2, 3, 4].map((i) => (
        <div
          key={i}
          className={`w-2 h-2 rounded-full transition-all duration-300 ${i <= score ? "bg-[#fc4c02]" : "bg-gray-200 dark:bg-gray-700"}`}
        />
      ))}
      <span
        className="ml-1 min-w-[14px] text-[12px] font-semibold text-gray-700 dark:text-gray-200 tabular-nums"
  ...
  return (
    <div className="mb-6">
      <div className="mb-4 flex items-center justify-between gap-4">
        <div
          className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-[11px] font-semibold ${color}`}
          style={{ fontFamily: '"Geometrica", sans-serif' }}
        >
          {title}
        </div>
      ...
      <div className="w-full overflow-hidden rounded-2xl border border-gray-200/60 bg-white shadow-[inset_0_1px_4px_rgba(0,0,0,0.06)] dark:border-white/5 dark:bg-gray-800">
        <div
          className={`grid ${columns} items-center border-b border-gray-200/60 bg-gray-50/80 dark:border-white/5 dark:bg-gray-900/60`}
        >
          <div
            className="px-5 py-4 text-left text-[11px] font-semibold text-gray-500 dark:text-gray-400"
            style={{ fontFamily: '"Geometrica", sans-serif' }}
          >
            Competency
          </div>
          <div
            className="px-5 py-4 text-center text-[11px] font-semibold text-gray-500 dark:text-gray-400"
            style={{ fontFamily: '"Geometrica", sans-serif' }}
          >
            Expected
          </div>
        ...
        {rows.map((row, i) => {
          ...
          return (
            <div
              key={i}
              className={`grid ${columns} min-h-[64px] items-center border-b border-gray-100 bg-white transition-colors duration-150 last:border-b-0 hover:bg-gray-50/60 dark:border-white/5 dark:bg-gray-800 dark:hover:bg-gray-700/30`}
            >
              <div
                className="px-5 py-4 text-left text-[12px] font-medium text-gray-800 dark:text-gray-200"
                style={{ fontFamily: '"Geometrica", sans-serif' }}
              >
                {row.name}
              </div>
              ...
              <div className="px-5 py-4 flex items-center justify-center">
                <span
                  className={`min-w-[24px] text-center text-[12px] font-bold tabular-nums ${gap < 0 ? "text-rose-500" : gap > 0 ? "text-emerald-500" : "text-blue-500"}`}
                  style={{ fontFamily: '"Geometrica", sans-serif' }}
                >
                  {gap > 0 ? `+${gap}` : gap}
                </span>
              ...
              <div className="px-5 py-4 flex items-center justify-start">
                <span
                  className={`inline-flex w-fit items-center gap-1.5 whitespace-nowrap rounded-full px-2.5 py-1 text-[11px] font-medium ${dev.color} ${dev.bg}`}
                  style={{ fontFamily: '"Geometrica", sans-serif' }}
                >
                  {dev.icon}
                  {dev.label}
...
export default function MyIDPPage({ role = "employee" }: MyIDPPageProps) {
  ...
  const categoryOptions: {
    ...
    ...(role === "manager"
      ? [
          {
            label: "Managerial" as ViewCategory,
  ...
  const selectedOption =
    categoryOptions.find((c) => c.label === selectedCategory) ??
    categoryOptions[0];
  ...
  return (
    <div className="flex-1 overflow-auto bg-[#f8fafc] dark:bg-transparent">
      <div className="mx-auto max-w-5xl p-6">
        <div style={fadeUp(0)}>
          <h1
            className="mb-1 text-left text-[22px] font-semibold text-gray-900 dark:text-white"
            style={{ fontFamily: '"Geometrica", sans-serif' }}
          >
            My IDP
          </h1>
        ...
        <div className="mb-8 grid grid-cols-3 gap-4" style={fadeUp(80)}>
          {[
            ...
            <div
              key={s.label}
              className={`flex items-center gap-3 rounded-2xl border p-4 ${s.bg} ${s.border}`}
            >
        ...
        <div
          className="mb-6 flex flex-wrap items-center gap-4 text-[11px] text-gray-500 dark:text-gray-400"
          style={fadeUp(120)}
        >
            ...
            <div key={s} className="flex items-center gap-1.5">
              <div className="flex gap-0.5">
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className={`h-2 w-2 rounded-full ${i <= s ? "bg-[#fc4c02]" : "bg-gray-200 dark:bg-gray-700"}`}
                  />
                ))}
              </div>
          ...
          <CompetencyTable
            ...
            extra={
              <select
                ...
                onChange={(e) =>
                  setSelectedCategory(e.target.value as ViewCategory)
                }
