...
export function AppSidebar({ onNavigate, username = "Tarin.Chon", position = "Fullstack Dev", role = "employee", onLogout, ...props }: AppSidebarProps) {
  ...
  if (role === 'admin') {
    ...
    generalItemsForRole = generalItems.filter(item => item.title !== 'Assessment');
    ...
    <Sidebar {...props} className="border-r border-gray-200 dark:border-gray-700">
      <SidebarHeader className="p-6 border-b border-gray-200 dark:border-gray-700">
        <img 
          ...
          style={{ filter: 'brightness(0) saturate(100%) invert(35%) sepia(90%) saturate(3500%) hue-rotate(200deg) brightness(100%) contrast(105%)' }}
        />
        <img 
          src={logoWhite}
          alt="1Moby" 
