...
const TEAM: TeamMember[] = [
  { id: "tarin",   firstName: "Tarin",   lastName: "Chon",  gradeCode: "EN 2", position: "Full Stack Dev",    level: 2, avatarColor: "#e05c4b", initials: "TC", competencyScore: 3.6, weeklyGain: 2.5, evaluated: false },
  { id: "emma",    firstName: "Emma",    lastName: "Horg",  gradeCode: "EN 1", position: "UX/UI",             level: 1, avatarColor: "#c4956a", initials: "EH", competencyScore: 3.2, weeklyGain: 2.5, evaluated: false },
  { id: "sarah",   firstName: "Sarah",   lastName: "Nigh",  gradeCode: "EN 2", position: "Business Analyst",  level: 3, avatarColor: "#4a8fa8", initials: "SN", competencyScore: 3.8, weeklyGain: 2.5, evaluated: true  },
  { id: "jessica", firstName: "Jessica", lastName: "Kenn",  gradeCode: "EN 1", position: "UX/UI",             level: 2, avatarColor: "#7a5c4f", initials: "JK", competencyScore: 3.2, weeklyGain: 2.5, evaluated: false },
  { id: "dustin",  firstName: "Dustin",  lastName: "Avia",  gradeCode: "EN 1", position: "Full Stack Dev",    level: 3, avatarColor: "#b08050", initials: "DA", competencyScore: 3.6, weeklyGain: 2.5, evaluated: false },
  { id: "jacob",   firstName: "Jacob",   lastName: "Mola",  gradeCode: "EN 1", position: "Full Stack Dev",    level: 2, avatarColor: "#5a7260", initials: "JM", competencyScore: 3.2, weeklyGain: 2.5, evaluated: true  },
];
...
const DEFAULT_SECTIONS: CompetencySection[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      { name: "FC02 · Stakeholder Communication", expected: 3, managerScore: 0 },
...
function getDevelopmentTag(gap: number) {
  if (gap > 0)  return { label: "Strength",       icon: "↗", color: "text-emerald-600 bg-emerald-50 border-emerald-200" };
  if (gap === 0) return { label: "Competency Fit", icon: "—", color: "text-blue-600 bg-blue-50 border-blue-200" };
  return             { label: "Critical",          icon: "↘", color: "text-rose-600 bg-rose-50 border-rose-200" };
}
        ...
        <button
          ...
          style={{
            background: i < display
              ? (interactive && hovered !== null ? (i < hovered ? "#fc4c02" : "#e5e7eb") : color)
              : "#e5e7eb",
          }}
        />
      ))}
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-[18px] font-semibold text-gray-800" style={GEO}>Assessment submitted!</p>
        <p className="text-[13px] text-gray-400" style={GEO}>Redirecting back…</p>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-4 p-5 mb-6 rounded-2xl bg-white dark:bg-gray-800 border border-gray-200/60 dark:border-white/5 shadow-[inset_0_1px_4px_rgba(0,0,0,0.06)]">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[17px] font-bold text-gray-900 dark:text-white" style={GEO}>
              {member.firstName}.{member.lastName}
            </span>
            <span className="text-[12px] text-gray-400" style={GEO}>({member.gradeCode})</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full text-[11px] font-semibold text-white" style={{ background: "#fc4c02" }}>
              Level {member.level}
            </span>
            <span className="px-2.5 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-full text-[11px] font-medium" style={GEO}>
              {member.position}
            </span>
          </div>
        </div>
        ...
        {sections.map((section, sIdx) => (
          <div key={section.label}>
            ...
            <div className="flex justify-center mb-4">
              <span className={`px-4 py-1 rounded-full text-[13px] font-semibold ${section.badgeColor}`} style={GEO}>
                {section.label}
              </span>
            </div>
            ...
            <div className="rounded-2xl bg-white dark:bg-gray-800 border border-gray-200/60 dark:border-white/5 shadow-[inset_0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden">
              ...
              <div className="grid grid-cols-[1fr_160px_160px_70px_140px] gap-2 px-5 py-3 border-b border-gray-100 dark:border-white/5 bg-gray-50/60 dark:bg-gray-900/30">
                {["Competency", "Expected", "Manager", "GAP", "Development"].map(h => (
                  <span key={h} className="text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide text-left" style={GEO}>
                    {h}
                  </span>
                ))}
              </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[13px] font-semibold text-gray-700 dark:text-gray-300" style={GEO}>
                        {row.expected}
                      </span>
                    </div>
                    ...
                    {/* GAP */}
                    <span className={`text-[13px] ${row.managerScore > 0 ? getGapColor(gap) : "text-gray-300"}`} style={GEO}>
                      {row.managerScore > 0 ? (gap > 0 ? `+${gap}` : gap) : "—"}
                    </span>

                    {/* Development */}
                    {row.managerScore > 0 ? (
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold border w-fit ${tag.color}`} style={GEO}>
                        <span>{tag.icon}</span>
                        {tag.label}
                      </span>
                    ) : (
                      <span className="text-[11px] text-gray-300 italic" style={GEO}>Set score</span>
                    )}
                  </div>
                );
              })}
...
export default function TeamAssessment() {
  ...
  const handleSubmit = (memberId: string) => {
    setTeam(prev => prev.map(m => m.id === memberId ? { ...m, evaluated: true } : m));
    setSelected(null);
  };
    ...
    <div className="p-6 bg-[#f8fafc] dark:bg-transparent min-h-screen" style={GEO}>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        ...
        <div className="flex text-left items-center gap-4 p-5 rounded-2xl bg-white dark:bg-gray-800 border border-gray-200/60 dark:border-white/5 shadow-[inset_0_1px_4px_rgba(0,0,0,0.06)]">
          ...
          <div>
            <p className="text-[28px] font-bold text-rose-500 leading-none" style={GEO}>{notEvaluated}</p>
            <p className="text-[12px] text-gray-500 dark:text-gray-400 mt-0.5" style={GEO}>Haven't Evaluated</p>
          </div>
        </div>
        ...
        <div className="flex text-left items-center gap-4 p-5 rounded-2xl bg-white dark:bg-gray-800 border border-gray-200/60 dark:border-white/5 shadow-[inset_0_1px_4px_rgba(0,0,0,0.06)]">
          ...
          <div>
            <p className="text-[28px] font-bold text-blue-500 leading-none" style={GEO}>{total}</p>
            <p className="text-[12px] text-gray-500 dark:text-gray-400 mt-0.5" style={GEO}>Total Team Members</p>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                ...
                {m.evaluated && (
                  <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 text-[10px] font-semibold border border-emerald-200/60 dark:border-emerald-800/30" style={GEO}>
